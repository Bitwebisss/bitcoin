...
Below are some notes on how to build Bitweb Core for Windows.

The options known to work for building Bitweb Core on Windows are:
...
example /usr/src/bitweb, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitweb`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/bitweb
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/bitweb --strip
```
