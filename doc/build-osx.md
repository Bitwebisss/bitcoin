...
This guide describes how to build bitwebd, command-line utilities, and GUI on macOS.
...
These tools must be installed in order to build Bitweb Core from source.
...
### 4. Clone Bitweb repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Bitweb Core repository to a directory.
...
You can [deploy](#3-deploy-optional) a `.zip` containing the Bitweb Core application.
It is required that you have `python` and `zip` installed.

## Building Bitweb Core

### 1. Configuration

There are many ways to configure Bitweb Core, here are a few common examples:
...
Run the following in your terminal to compile Bitweb Core:
...
Bitweb Core should now be available at `./build/bin/bitwebd`.
If you compiled support for the GUI, it should be available at `./build/bin/bitweb-qt`.

There is also a multifunction command line interface at `./build/bin/bitweb`
supporting subcommands like `bitweb node`, `bitweb gui`, `bitweb rpc`, and
others that can be listed with `bitweb help`.

The first time you run `bitwebd` or `bitweb-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Bitweb/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bitweb"

touch "/Users/${USER}/Library/Application Support/Bitweb/bitweb.conf"
