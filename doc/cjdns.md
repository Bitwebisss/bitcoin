# CJDNS support in Bitweb Core

It is possible to run Bitweb Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitweb Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitweb network and individual nodes.
...
before it will work with your Bitweb Core node. This is described in steps
...
## Run Bitweb Core with CJDNS

Once you are connected to the CJDNS network, the following Bitweb Core
...
When enabled, this option tells Bitweb Core that it is running in an
...
IPv6 local network. This helps Bitweb Core perform better address management:
...
CJDNS peers than Tor or IP ones. You can use `bitweb-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/Bitwebissss/bitweb/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bitweb Core

There are several ways to see your CJDNS address in Bitweb Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitweb-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitweb-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitweb-cli help getnodeaddresses` for details.
