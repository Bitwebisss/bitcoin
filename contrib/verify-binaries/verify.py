...
"""Script for verifying Bitweb Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitwebcore.net and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/Bitwebissss/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Bitweb community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"bitweb_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by bitwebcore.net
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the bitweb release to download; of the format '
