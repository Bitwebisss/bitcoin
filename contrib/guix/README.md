# Bootstrappable Bitweb Core Builds

This directory contains the files necessary to perform bootstrappable Bitweb
Core builds.
...
To build Bitweb Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bitweb Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitweb-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/Bitwebissss/guix.sigs).
